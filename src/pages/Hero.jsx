      ...
      <div className="max-w-7xl mx-auto w-full relative z-10">
        ...
        >
          ...
          <div className="flex-1 max-w-2xl text-center md:text-left">
           {/* Animated Logo (entry animation with pop-in) */}
<motion.div
  initial={{ opacity: 0, scale: 0.5 }}
  animate={{ opacity: 1, scale: 1 }}
  transition={{ duration: 0.8, ease: "easeOut" }}
  className="mb-6 w-24 h-24 mx-auto md:mx-0 rounded-full bg-gradient-to-br from-blue-500 via-purple-500 to-pink-500 p-1 shadow-2xl flex items-center justify-center"
>
  <div className="w-full h-full rounded-full bg-white dark:bg-gray-900 flex items-center justify-center text-3xl font-bold relative overflow-hidden">
    <span className="relative z-10 bg-clip-text text-transparent bg-gradient-to-r from-blue-500 to-purple-500">
      KR
            ...
            {/* Typing Effect */}
            <motion.div variants={itemVariants} className="mb-4 h-7">
            ...
            <motion.p
              variants={itemVariants}
              className="text-base md:text-lg text-black/60 dark:text-white/60 leading-relaxed mb-6"
            >
              Crafting robust, scalable web solutions with cutting-edge technologies. From concept to deployment, I bring your digital vision to life with precision and passion.
            </motion.p>

            {/* Availability Badge */}
            <motion.div
              variants={itemVariants}
              className="inline-flex items-center gap-3 px-6 py-3 rounded-full bg-emerald-500/10 border border-emerald-500/30 backdrop-blur-sm mb-6"
            >
              <motion.div
          ...
          {/* Right Column: Service Steps without heading */}
          <motion.div className="flex-shrink-0 w-full md:w-auto md:max-w-sm flex flex-col gap-6" variants={itemVariants}>
            <div className="relative flex flex-col gap-6">
              <div className="absolute left-5 top-5 bottom-5 w-0.5 bg-white/20" />
              {services.map((service, index) => (
                ...
                >
                  <div className={`flex-shrink-0 w-10 h-10 rounded-full bg-white/10 dark:bg-gray-800 backdrop-blur-sm border border-white/10 flex items-center justify-center ${service.color}`}>
                    {service.icon}
                  </div>
                  <div className="text-left">
                    <h4 className="font-bold mb-0.5">{service.title}</h4>
